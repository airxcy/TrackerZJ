...
//using namespace itf;  // NOLINT(build/namespaces)
...
*/
template<typename Dtype>
int feature_extraction_pipeline_new(char* pretrained_net_param,
										  char* feature_extraction_proto_file,
										  char* extract_feature_blob_name,
										  char* save_feature_leveldb_name,
										  ...
										  uint 	device_id = 0)
{
  if (strcmp(CPU_or_GPU, "GPU") == 0)
  {
    ...
    Engine::SetDevice(device_id);
    Engine::set_mode(Engine::GPU);
  } else {
    LOG(ERROR) << "Using CPU";
    Engine::set_mode(Engine::CPU);
  }
  //Engine::set_phase(itf::TEST);

  //arg_pos = 0;  // the name of the executable
  string pretrained_binary_proto(pretrained_net_param);
   ...
   */
  string feature_extraction_proto(feature_extraction_proto_file);
  shared_ptr<Net<Dtype> > feature_extraction_net(
      new Net<Dtype>(feature_extraction_proto, itf::TEST));
  feature_extraction_net->CopyTrainedLayersFrom(pretrained_binary_proto);

  string extract_feature_blob_names(extract_feature_blob_name);
  vector<string> blob_names;
  boost::split(blob_names, extract_feature_blob_names, boost::is_any_of(","));

  string save_feature_leveldb_names(save_feature_leveldb_name);
  ...
  for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index) {
    ...
    for (int i = 0; i < num_features; ++i) {
      ...
      for (int n = 0; n < batch_size; ++n) {
        ...
        snprintf(key_str, kMaxKeyStrLength, "%8d", image_indices[i]);
